...
public class VirtualMachine {
		...
		for(int i = 0; i < 500; i++) {
			...
			if(instructionExeciutor(this.workMemory.get(workingIndex))) {
				...
				if(this.map.gold.isEmpty()) {
					this.finder.fitness += (10000 - this.numberOfMoves);
					return true;
				}
			}else {
	...
	public boolean moveExeciutorPrint() {
		checkForGold();
		this.map.mapSetOn(this.indexHeight, this.indexWidth, Integer.toString(this.finder.fitness/this.oneGoldPrice));
		for(int i = 0; i < this.movesOnMap.size(); i++) {
			...
			//vypis mapy
			System.out.println();
			System.out.println("Fitness: " + Integer.toString(this.finder.fitness) + " Gold Found: " + Integer.toString(this.finder.fitness/this.oneGoldPrice));
